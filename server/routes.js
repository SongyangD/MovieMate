...
const search_oscar_people = async function(req, res) {
  ...
  //   WHERE O.category = "ACTOR" OR O.category ="ACTRESS" and O.imdb_title_id = MP.imdb_title_id and MP.imdb_name_id = P.imdb_name_id
  //   ORDER BY P.name ASC
  //   LIMIT ${offset}, ${pageSize};
  // `;
...
INNER JOIN oscar O ON O.imdb_title_id = MP.imdb_title_id
WHERE O.category IN ('BEST PICTURE')
ORDER BY P.name ASC
LIMIT ${offset}, ${pageSize};
  `;
...
//   FROM movie_data M
//   WHERE M.year = ${year} and M.country LIKE '%${country}$'
//   and M.imdb_title_id 
//   IN 
...
//   var query_no_year = `
//   SELECT  O.category, M.imdb_title_id, M.title, M.country, M.year, M.genre, M.avg_vote, O.winner
...
//   var query = `
//   SELECT DISTINCT O.category, M.imdb_title_id, M.title, M.country, M.year, M.genre, M.avg_vote, O.winner
//   FROM movie_data M JOIN oscar O ON M.imdb_title_id = O.imdb_title_id
//   WHERE M.year = ${year}
//   and O.winner = 1
//   Order by M.title;
...
// their movies have received, and the number of movies they directed 
// top5
